"""
presets.py - Hand-drawn room renderers (ported from the midterm project).

Each function below takes (canvas, x, y, w, h) in absolute canvas coords
and paints a fully-detailed room interior.  These renderers are used by
the seeded RoomTemplates (preset_id in {"BedroomA", ..., "ReadingRoom"})
so they stay visually identical to the midterm while fitting into the
new polygon / library data model.
"""
...
def _proc_arc(c, cx, cy, w, h, start_rad, stop_rad, outline=BLK, width=1):
    x0 = cx - w / 2; y0 = cy - h / 2
    x1 = cx + w / 2; y1 = cy + h / 2
...
def draw_bedroom_A(c, x, y, rw, rh):
    A_BASE = 400.0
    sc = rw / A_BASE
    wall = WALL; inn = INNER
...
def draw_bedroom_B(c, x, y, rw, rh):
    B_BASE = 500.0
    ...
    for frac in (0, 1 / 3, 2 / 3, 1):
        _line(c, win_x2 + (wall - 2) * frac, div_y2,
              win_x2 + (wall - 2) * frac, y + rh - wall)
...
def draw_bedroom_C(c, x, y, rw, rh):
    rs = min(rw, rh)
    wall = rs * 0.5 / 9
...
def draw_bedroom_D(c, x, y, rw, rh):
...
def draw_tea_room_2(c, x, y, w, h):
    ...
    _rect(c, x + w - t - cw2 + (cw2 - cw2 * 0.55) / 2, y + t + ch2 * 0.70,
          cw2 * 0.55, ch2 * 0.18)
    d = int(min(w, h) * 0.18)
...
def draw_reading_room(c, x, y, rw, rh):
    ...
    for i in range(46):
        _rect(c, x + i * rw * 0.25 / 12.5 + ww, y + wh + rh * 10.5 / 12.5,
              rw * 0.25 / 12.5, rh / 12.5)
    ...
    for txf in (0.5 / 12.5, 3 / 12.5, 5.5 / 12.5):
        tx = x + rw * txf; ty = y + rh * 0.5 / 12.5
        tw2 = rw * 2.5 / 12.5; th2 = rh * 2 / 12.5
        _rect(c, tx, ty, tw2, th2)
        r_ch = rw * 0.5 / 12.5
...
def draw_library(c, x, y, rw, rh):
    ...
    _line(c, lx + rw * 12.25 / 26, ly + rh * 0.75 / 17,
          lx + rw * 12.25 / 26, ly + rh * 5.75 / 17)
    r_s = rw * 0.5 / 26
    ...
    draw_reading_room(c, x + rw * 13.5 / 26, y, rw * 12.5 / 26, rh * 12.5 / 17)
...
PRESET_RENDERERS: Dict[str, Callable] = {
    "BedroomA":    draw_bedroom_A,
    "BedroomB":    draw_bedroom_B,
    "BedroomC":    draw_bedroom_C,
    "BedroomD":    draw_bedroom_D,
    "TeaRoom1":    draw_tea_room_1,
    "TeaRoom2":    draw_tea_room_2,
    "Library":     draw_library,
    "ReadingRoom": draw_reading_room,
}
